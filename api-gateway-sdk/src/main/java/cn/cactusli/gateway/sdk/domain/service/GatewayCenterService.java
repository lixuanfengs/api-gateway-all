package cn.cactusli.gateway.sdk.domain.service;

import cn.cactusli.gateway.sdk.GatewayException;
import cn.cactusli.gateway.sdk.common.Result;
...
public class GatewayCenterService {
    ...
    public void doRegisterApplication(String address, String systemId, String systemName, String systemType, String systemRegistry) {
        ...
        String resultStr;
        try {
            resultStr = HttpUtil.post(address + "/cactus/admin/register/registerApplication", paramMap, 5500);
        } catch (Exception e) {
            logger.error("应用服务注册异常，链接资源不可用：{}", address + "/cactus/admin/register/registerApplication");
            throw e;
        }
    ...
    public void doRegisterApplicationInterface(String address, String systemId, String interfaceId, String interfaceName, String interfaceVersion) {
        ...
        String resultStr;
        try {
            resultStr = HttpUtil.post(address + "/cactus/admin/register/registerApplicationInterface", paramMap, 5500);
        } catch (Exception e) {
            logger.error("应用服务接口注册异常，链接资源不可用：{}", address + "/cactus/admin/register/registerApplicationInterface");
            throw e;
        }
        Result<Boolean> result = JSON.parseObject(resultStr, new TypeReference<Result<Boolean>>() {
        });
    ...
    public void doRegisterApplicationInterfaceMethod(String address,
        ...
        String resultStr;
        try {
            resultStr = HttpUtil.post(address + "/cactus/admin/register/registerApplicationInterfaceMethod", paramMap, 5500);
        } catch (Exception e) {
            logger.error("应用服务接口注册方法异常，链接资源不可用：{}", address + "/cactus/admin/register/registerApplicationInterfaceMethod");
            throw e;
        }
        Result<Boolean> result = JSON.parseObject(resultStr, new TypeReference<Result<Boolean>>() {
        });
    ...
    public void doRegisterEvent(String address, String systemId) {
        ...
        String resultStr;
        try {
            resultStr = HttpUtil.post(address + "/cactus/admin/register/registerEvent", paramMap, 5500);
        } catch (Exception e) {
            logger.error("应用服务接口事件方法异常，链接资源不可用：{}", address + "/cactus/admin/register/registerEvent");
            throw e;
        }
        Result<Boolean> result = JSON.parseObject(resultStr, new TypeReference<Result<Boolean>>() {
        });
